...
from .utils import sentence_to_ids, ids_to_sentence, batch_to_ids


class Seq2SeqModel(object):
    """Seq2Seq model with attention on decoder."""
    def __init__(self, size_memory, word2id, id2word, max_decode):
        self.memory_list = []
    ...
    def declare_placeholders(self):
        self.input_batch = tf.compat.v1.placeholder(shape=(None, None), dtype=tf.int64, name='input_batch')
        self.input_batch_lengths = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='input_batch_lengths')

        self.ground_truth = tf.compat.v1.placeholder(shape=(None, None), dtype=tf.int64, name='ground_truth')
        self.ground_truth_lengths = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='ground_truth_lengths')

        self.memory_batch = tf.compat.v1.placeholder(shape=(None, None), dtype=tf.int64, name='memory')
        self.memory_batch_lengths = tf.compat.v1.placeholder(shape=(None,), dtype=tf.int32, name='memory_lengths')
        ...
        self.mem_size = tf.compat.v1.placeholder(dtype=tf.int64, shape=[])
        self.mode = tf.compat.v1.placeholder(dtype=tf.int64, shape=[])

    def create_embeddings(self, emb_size):
        random_initializer = tf.compat.v1.random_uniform((len(self.word2id.keys()), emb_size), -1.0, 1.0)
        self.embeddings = tf.Variable(initial_value=random_initializer, dtype=tf.float32)
        self.input_batch_embedded = tf.compat.v1.nn.embedding_lookup(self.embeddings, self.input_batch)
    ...
    def build_encoder(self, hidden_size):
        ...
        self.encoder_out, self.final_encoder_state = tf.nn.dynamic_rnn(encoder_cell, self.input_batch_embedded,
    ...
    def build_decoder(self, hidden_size, vocab_size, start_symbol_id, end_symbol_id):
        ...
        def decode(helper, scope, reuse=None):
            with tf.compat.v1.variable_scope(scope, reuse=reuse):
                attention_mechanism = tf.contrib.seq2seq.LuongAttention(num_units=hidden_size, memory=self.encoder_out,
                ...
                state = tf.cond(self.mem_size > 0,
                                lambda: decoder_cell.zero_state(tf.shape(self.input_batch)[0], tf.float32).clone(
                                    cell_state=tf.add(self.final_encoder_state, self.memory_out)),
                                lambda: decoder_cell.zero_state(tf.shape(self.input_batch)[0], tf.float32).clone(
                                    cell_state=self.final_encoder_state))

                decoder = tf.contrib.seq2seq.BasicDecoder(cell=decoder_cell, helper=helper, initial_state=state)

                def dynamic_decode_training():
                    outputs, _, _ = tf.contrib.seq2seq.dynamic_decode(decoder=decoder, output_time_major=False,
                                                                      impute_finished=True)
                    return outputs

                def dynamic_decode_infer():
                    outputs, _, _ = tf.contrib.seq2seq.dynamic_decode(decoder=decoder, output_time_major=False,
                                                                      impute_finished=True, maximum_iterations=self.max_decode)
                    return outputs
    ...
    def memory_network(self, hidden_size):
        encoder_cell_input = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(hidden_size),
                                                           input_keep_prob=self.dropout_ph, dtype=tf.float32)
        encoder_cell_memory = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(hidden_size),
                                                            input_keep_prob=self.dropout_ph, dtype=tf.float32)

        self.memory_batch_embedded = tf.nn.embedding_lookup(self.embeddings, self.memory_batch)

        self.dense_layer = tf.compat.v1.layers.Dense(hidden_size)
        _, self.final_memory_encoder_state = tf.nn.dynamic_rnn(encoder_cell_memory, self.memory_batch_embedded,
                                                               sequence_length=self.memory_batch_lengths,
                                                               dtype=tf.float32, scope="rnn2")
        _, self.final_input_encoder_state = tf.nn.dynamic_rnn(encoder_cell_input, self.input_batch_embedded,
                                                              sequence_length=self.input_batch_lengths,
        ...
        def train():
            def train_with_memory():
                final_memory_encoder_state = tf.reshape(self.final_memory_encoder_state,
                                                        [tf.shape(self.input_batch)[0], -1, hidden_size])
                matrix_product = tf.matmul(final_memory_encoder_state[:][:][0],
                                           tf.expand_dims(self.final_input_encoder_state[0], 1))
                ...
                def body(num, H):
                    matrix_product = tf.matmul(final_memory_encoder_state[:][:][num],
                                               tf.expand_dims(self.final_input_encoder_state[num], 1))
                    P = tf.nn.softmax(tf.transpose(matrix_product))
                ...
                _, H1 = tf.while_loop(condition, body, loop_vars=[1, H], shape_invariants=[tf.constant(1).get_shape(),
                                                                                           tf.TensorShape(
                                                                                               [None, hidden_size])])
        ...
        def predict():
            def predict_with_memory():
                matrix_product = tf.matmul(self.final_input_encoder_state,
                                           tf.transpose(self.final_memory_encoder_state))
    ...
    def perform_optimization(self):
        ...
        self.train_op = tf.contrib.layers.optimize_loss(self.loss, optimizer="Adam",
                                                        ...
                                                        clip_gradients=clip_gradients_by_l2)

    def train_on_batch(self, session, X, X_seq_len, Y, Y_seq_len, mem_batch, mem_batch_len, memory, learning_rate,
                       dropout_keep_probability, mode):
        feed_dict = {}
        if mode == 1:
            feed_dict = {
                self.input_batch: X,
