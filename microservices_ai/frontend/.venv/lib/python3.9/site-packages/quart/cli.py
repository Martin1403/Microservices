...
@click.command("router", short_help="Show this app's router.")
@click.option(
    "--sort",
    "-s",
    type=click.Choice({"endpoint", "methods", "rule", "match"}),  # type: ignore
    default="endpoint",
    help="Order the router by type, 'match' is the matching order when dispatching a request.",
)
...
def routes_command(info: ScriptInfo, sort: str, all_methods: bool) -> None:
    ...
    if len(rules) == 0:
        click.echo("No router were registered.")
        return
